...
Codex 会根据第一句话生成标题，但工作方向可能越走越远。后续目标真正做出可靠结果、而且工作章节明显变化时，Continuity 会通过当前 Codex 直接刷新原生标题，同时在本地留下简短进展和原标题。自动改名可以查看、撤销、暂停，也可以重新开启。
...
只有工作足够独立，而且并行确实更快或更稳时，Continuity 才会建议子智能体。你已经授权自动委派时，它可以直接派遣并简单说明；没有授权时，它会等你选择「并行处理」或「就在这里做」。主代理是否切换仍由你决定。模型建议可以参考 [ModelDial Radar](https://modeldial.com/radar) 的公开数据，但不会把你的请求、代码、任务标题、项目目录、当前配置或凭据发送给 ModelDial。
...
- 查天气、改一句话这类临时问题不需要放进项目。没有可靠的项目目录时，Continuity 不会乱猜。
...
Continuity 复用两个官方时机：`UserPromptSubmit` 让当前 Codex 静默判断后续目标，并在可靠结果进入新章节时调用当前宿主的原生标题工具；兼容型 `Stop` Hook 负责记录进展和持久化兜底。Stop 入口启动本地后台 worker 后立即返回，不依赖 Codex runtime 是否支持异步 Hook。macOS 使用插件自带的 Shell 入口；Windows 使用官方 `commandWindows`、原生 PowerShell，并优先复用 Desktop 内置的 Node／Codex runtime。
...
- **本地只留短记录：**保存任务 ID、简短主题、简短进展和控制状态，不保存完整请求或整段对话。
...
| 你提出后续目标 | `UserPromptSubmit`、路由 Skill、当前 Codex 模型和原生工具 | 判断继续、并行、开支线还是新建；当前任务做出可靠结果后只刷新变化的章节 | 留在当前任务并保留标题 |
