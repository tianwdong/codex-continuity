...
选型先过滤宿主明确支持的模型和档位，再按质量门槛与参考成本比较，不再固定绑定 Luna／Terra／Sol。固定 agent profile 保持不变。综合排名使用自己的批次标识；API 渠道数据仅作为跨渠道参考，不等于原生登录表现或订阅成本。当前子代理配置未测或未知时会明确标注，选型失败会说明降级原因。是否并行还要看独立产物、主任务能否同时推进以及验收成本，不能只凭“文件多”就拆任务。

子智能体只完成限定的小块工作；结果回到当前任务后，仍由主代理核验，再统一交付给你。
...
单独的“你好”等明确问候直接回答，不查任务、不占用首次匹配机会。翻译、改写等一次性请求由当前 Codex 在查任务之前判断，不另起分类模型。

连续几轮快速结束时，后台会合并等待中的整理请求并补到最新一轮。处理失败后，已排队的 worker 保留待处理坐标，等新的 Stop 投递再尝试，避免连续重复调用。整理期间仍可查看上次可靠进展和刷新状态。后台进程被系统终止或对应最终回复暂不可读时，不承诺已经补齐。

“上次做到哪，还有什么没做”会给出当前任务的续接简报：已完成结果、明确未完成的工作或阻塞、仍需确认的决定，并标明依据。优先使用当前对话和进展记录；证据不足时，最多读取当前任务两页、每页三轮的近期记录。没有依据就标为未知，普通“继续”不会额外触发检查。

“检查 Continuity 是否正常”只读取本地状态，报告执行插件包版本、最近 Stop 诊断记录的代码版本、最近收到和处理的回合、进展新鲜度、后台锁与失败阶段，并给出对应处理建议。它不调用模型，也不把这两个版本或缺少记录当作宿主当前加载版本、Hook 信任状态的证明。启动失败记录不能证明后台 worker 已运行。
...
构建包含可重现的 `build-integrity.json`。doctor 分别报告磁盘包内容一致性和最近 Stop 记录来源指纹，两者都不证明宿主已加载该包。`received_stop_handled` 与 `matches_received_stop` 仅针对已收到的 Stop，本地诊断对最新原生完成回合的覆盖始终标为未核验。同一目标的继续不重新路由，尚未解决的选择优先处理。
